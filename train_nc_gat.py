...
graph_name_list = ['Cora', 'CiteSeer', 'PubMed', 'Photo', 'Physics', 'Cora_ML', 'Minesweeper', 'DBLP', 'Roman-empire',
                   'Amazon-ratings']

graph_name_list = ['Europe', 'USA', 'Cora', 'Cora_ML', 'CiteSeer', 'Wiki']
# graph_name_list = ['PubMed','Physics']
# Cora
...
class GAT(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = GATConv(len(X[0]), hidden_layers)
        self.conv2 = GATConv(hidden_layers, hidden_layers)
        self.conv3 = GATConv(hidden_layers, classes_count)
...
for graph_id, graph_name in enumerate(graph_name_list):
            ...
            if (k == 26):
                ...
                for features_count in range(k, k + 1):
                    ...
                    for iteration in range(1, 2):
                        ...
                        gat = GAT()
                        optimizer_gat = torch.optim.Adam(gat.parameters(), lr=0.001, weight_decay=5e-4)
                        criterion = nn.CrossEntropyLoss()
                        gat = train_node_classifier(gat, graph, X, optimizer_gat, criterion)

                        test_acc_metrics = eval_node_classifier(gat, graph, X, graph.test_mask)
                        ...
                        writeToReport(report_file, 'GAT on metrics: ' + str(np.round(test_acc_metrics, 3)) + '\n')
    ...
    if (vanilla_train):
        ...
        criterion = nn.CrossEntropyLoss()
        gat = train_node_classifier(gat, graph, X, optimizer_gat, criterion)
        test_acc_vanilla = eval_node_classifier(gat, graph, X, graph.test_mask)
        print(f'GAT on vanilla, Test Acc: {test_acc_vanilla:.3f}')
        writeToReport(report_file, 'GAT on vanilla: ' + str(np.round(test_acc_vanilla, 3)) + '\n  \n')
